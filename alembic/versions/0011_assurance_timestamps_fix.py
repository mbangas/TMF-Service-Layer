"""Fix missing server_default on created_at / updated_at for assurance tables.

Migration 0006 created alarm, performance_measurement and service_level_objective
...
auto-populated them on INSERT.  This migration:
  ...
  2. Adds ``DEFAULT NOW()`` to both columns on all three tables.
  3. Tightens the columns to NOT NULL so future INSERTs are always stamped.

Revision ID: 0011_assurance_timestamps_fix
Revises: 0010_service_dependencies
...
revision: str = "0011_assurance_timestamps_fix"
down_revision: str = "0010_service_dependencies"
branch_labels = None
depends_on = None

_TABLES = ("alarm", "performance_measurement", "service_level_objective")
